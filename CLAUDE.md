...
Only two `eslint-disable-next-line` comments were added, each saying what
re-running would break. Two scoped overrides in `eslint.config.js` turn
`react-refresh/only-export-components` off for `src/components/ui/**`
(regenerated by the shadcn CLI, which ships the variant exports) and
`src/contexts/**` (a provider beside its `useX` hook is the idiom). The rule is
live everywhere else, so a new warning is a build failure: fix it, or add a
disable that says why. Do not reach for `--max-warnings` to make one go away.
