...
export const MonthlyCalendarScreen: React.FC<Props> = ({
  ...
  // Scope future workouts to the active program (and manual entries) so a
  // previously-active program's schedule doesn't leak onto the calendar.
  const visibleFutureWorkouts = useMemo(
    () => futureWorkouts.filter(f => f.programId === activeProgram?.id || f.programId === 'manual'),
    [futureWorkouts, activeProgram],
  );
